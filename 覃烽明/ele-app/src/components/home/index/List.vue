...
<ul class="seller-list">
    <li class="seller-item one-bottom-px" v-for="(seller, index) in listData" :key="index" @click="goDetailPage(index)" :id="seller.id">
        <div class="logo">
            <img :src="seller.img"/>
        </div>
        <div class="info">
            ...
            <div class="activities">
                ...
                <span class="btn" @click.stop="showActAction(index)" v-show="seller.activities.length>2">{{seller.activities.length}}个活动</span>
            </div>
        </div>
    </li>
</ul>
...
export default {
    ...
    methods: {
        ...
        goDetailPage(index){
            this.$router.push('/home/detail'+'?'+'id='+this.listData[index].id);
        }
    },
    mounted(){
        this.$watch('lon',()=>{
